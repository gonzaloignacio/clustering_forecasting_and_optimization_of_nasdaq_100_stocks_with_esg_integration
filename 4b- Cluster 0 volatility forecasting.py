...
y_train = train_df["Rolling_Volatility"]
y_test = test_df["Rolling_Volatility"]
...
for ticker in ticker_c0:
    ...
    target_idx = list(scaler.feature_names_in_).index("Rolling_Volatility")
...
cluster_0_forecast_v = pd.DataFrame({"company": tickers,
                                   "forecast": forecast})

#Sort to get the top 10 and plot
top_c0v = cluster_0_forecast_v.sort_values("forecast", ascending = False).iloc[0:10, ]
sns.barplot(data = top_c0v, x = "forecast", y = "company")
plt.xlabel("Forecasted volatility")
plt.ylabel("Comapny")
plt.title("Top 10 forecasted volatility by company for cluster 0")
plt.xticks(rotation = 45)
plt.show()

#Sort to get the bottom 10 and plot
bottom_c0v = cluster_0_forecast_v.sort_values("forecast").iloc[0:10, ]
sns.barplot(data = bottom_c0v, x = "forecast", y = "company")
plt.xlabel("Forecasted volatility")
plt.ylabel("Comapny")
plt.title("Bottom 10 forecasted volatility by company for cluster 0")
plt.xticks(rotation = 45)
plt.show()

# Save it as a csv   
cluster_0_forecast_v.to_csv(r"C:\Users\gonza\Documents\cluster_0_forecast_v.csv")