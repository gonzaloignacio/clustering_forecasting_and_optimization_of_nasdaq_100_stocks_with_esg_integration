...
min_v_model.eval("param esg_threshold;") # esg_threshold
...
# Portfolio ESG score must be less or equal to the threshold
min_v_model.eval("""subject to maximum_esg:
                     sum{i in companies}
                         x[i] * esg[i] <= esg_threshold;""")
...
min_v_model.param["esg_threshold"] = esg_threshold
...
max_r_model.eval("param esg_threshold;") # ESG threshold
...
# Portfolio ESG score must be less or equal to the threshold value                         
max_r_model.eval("""subject to maximum_esg:
                     sum{i in companies}
                         x[i] * esg[i] <= esg_threshold;""")
...
max_r_model.param["esg_threshold"] = esg_threshold
...
min_esg_model.set["companies"] = companies

# Define parameters
min_esg_model.eval("param r{companies};") # Stock return
min_esg_model.eval("param v{companies, companies};") # Covariance matrix
min_esg_model.eval("param esg{companies};") # ESG score
min_esg_model.eval("param r_threshold;") # Return threshold
min_esg_model.eval("param v_threshold;") # Volatility threshold
min_esg_model.eval("param r_f;") # Risk free asset return

# Define variables
min_esg_model.eval("var x{companies} >= 0;") # Weights
min_esg_model.eval("var portfolio_v;") # Portfolio volatility
min_esg_model.eval("var portfolio_r;") # Portfolio return
min_esg_model.eval("var portfolio_esg;") # Portfolio ESG score

# Define objective function
min_esg_model.eval("""minimize portfolio_esgscore: 
                     portfolio_esg;""")

# Set portfolio volatility
min_esg_model.eval("""subject to set_portfolio_v:
                     sum{i in companies, j in companies}
                         x[i] * v[i, j] * x[j] = portfolio_v;""")

# Set portfolio return
min_esg_model.eval("""subject to set_portfolio_r:
                     sum{i in companies}
                         x[i] * r[i] = portfolio_r;""")

# Set portfolio ESG score                     
min_esg_model.eval("""subject to set_portfolio_esg:
                     sum{i in companies}
                         x[i] * esg[i] = portfolio_esg;""")

# The sum of weights must me equal to 1
min_esg_model.eval("""subject to weights_constraint:
                     sum{i in companies}
                         x[i] = 1;""")

# Portfolio volatility must be less or equal to the threshold                         
min_esg_model.eval("""subject to maximum_volatility:
                     sum{i in companies, j in companies}
                         x[i] * v[i,j] * x[j] <= v_threshold;""")
                         
# Portfolio return must be greater or equal to the threshold
min_esg_model.eval("""subject to minimum_return:
                     sum{i in companies}
                         x[i] * r[i] >= r_threshold;""")
    
# Assign dictionaries to parameters
min_esg_model.param["r"] = mean_dict
min_esg_model.param["v"] = cov_dict
min_esg_model.param["esg"] = esg_dict
min_esg_model.param["r_threshold"] = r_threshold
min_esg_model.param["v_threshold"] = v_threshold
min_esg_model.param["r_f"] = r_f

# Solve the model with a non-linear solver                    
min_esg_model.set_option("solver", "bonmin")
min_esg_model.solve()

# Store portfolio return, volatility, Sharpe ratio and ESG score
min_esg_volatility = np.sqrt(min_esg_model.get_variable("portfolio_v")[()].value())
min_esg_return= min_esg_model.get_variable("portfolio_r")[()].value()
min_esg_esg = min_esg_model.get_variable("portfolio_esg")[()].value()
min_esg_sharpe = (min_esg_return - r_f) / min_esg_volatility
min_esg_result = pd.DataFrame({"model": "min_esg_portfolio",
                              "return": [min_esg_return],
                             "volatility": [min_esg_volatility],
                             "sharpe": [min_esg_sharpe],
                             "esg": [min_esg_esg]})
min_esg_result = min_esg_result.set_index("model")

# Store the weights
x = min_esg_model.get_variable("x")
min_esg_weights = {c: x[c].value() for c in min_esg_model.get_set("companies")}
min_esg_weights_df = pd.DataFrame([min_esg_weights])
min_esg_weights_df = min_esg_weights_df.round(3)
min_esg_weights_df = min_esg_weights_df.div(min_esg_weights_df.sum(axis = 1), axis = 0)
min_esg_weights_df["model"] = "min_esg_portfolio"
min_esg_weights_df = min_esg_weights_df.set_index("model")

# Store indexes and weights together
...
max_sharpe_model.eval("param esg_threshold;") # Threshold ESG score
...
# Portfolio ESG score should be less or equal to the threshold
max_sharpe_model.eval("""subject to maximum_esg:
                     sum{i in companies}
                         x[i] * esg[i] <= esg_threshold;""")
...
max_sharpe_model.param["esg_threshold"] = esg_threshold
