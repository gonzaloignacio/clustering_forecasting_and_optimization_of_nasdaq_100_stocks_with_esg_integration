...
cluster_labels_df = cluster_labels_df[cluster_labels_df["Cluster"] == 0]
...
svr_selected = SVR(kernel = svr_best_kernel,
               C = svr_best_C,
               gamma = svr_best_gamma,
               epsilon = svr_best_epsilon)
svr_selected.fit(X_full, y_full)
...
for ticker in ticker_c0:
    ...
    y_pred_scaled = svr_selected.predict(pd.DataFrame([X_ticker]))
...
cluster_0_forecast_r = pd.DataFrame({"company": tickers,
                                   "forecast": forecast})

#Sort to get the top 10 and plot
top_c0r = cluster_0_forecast_r.sort_values("forecast", ascending = False).iloc[0:10, ]
sns.barplot(data = top_c0r, x = "forecast", y = "company")
plt.xlabel("Forecasted return")
plt.ylabel("Comapny")
plt.title("Top 10 forecasted return by company for cluster 0")
plt.xticks(rotation = 45)
plt.show()

#Sort to get the bottom 10 and plot
bottom_c0r = cluster_0_forecast_r.sort_values("forecast").iloc[0:10, ]
sns.barplot(data = bottom_c0r, x = "forecast", y = "company")
plt.xlabel("Forecasted return")
plt.ylabel("Comapny")
plt.title("Bottom 10 forecasted return by company for cluster 0")
plt.xticks(rotation = 45)
plt.show()

# Save it as a csv   
cluster_0_forecast_r.to_csv(r"C:\Users\gonza\Documents\cluster_0_forecast_r.csv")
